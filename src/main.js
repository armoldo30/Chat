...
function air(c){
  ...
  <section class="air-comparison-report"><div class="air-verdict"><span>COMPARATIVE RESULT</span><h2>${verdict}</h2><p>This is an analytical exchange model using attack/defense, agility, speed, reliability, detection and mission efficiency. Air-to-air exchange remains analytical. Source-slot aircraft data is active; exact NAir executable parity and repeated mission-specific module effects remain separately audited.</p></div><div class="air-report-metrics"><article><span>KILL RATIO A:B</span><strong>${kill}</strong><small>${fmt(r.lossB,2)} enemy / ${fmt(r.lossA,2)} friendly losses</small></article><article><span>IC EXCHANGE</span><strong>${exchange}:1</strong><small>${fmt(r.icLostB,0)} enemy IC / ${fmt(r.icLostA,0)} friendly IC lost</small></article><article><span>AIR POWER SHARE</span><strong>${fmt(r.airPowerShareA,1)}%</strong><small>friendly analytical share</small></article><article><span>${esc(effA.label)}</span><strong>${fmt(effA.score,2)}</strong><small>enemy ${fmt(effB.score,2)}</small></article></div></section>
