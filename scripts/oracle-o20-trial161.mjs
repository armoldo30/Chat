...
export const O20_HOURS=Object.freeze(Array.from({length:162},(_,i)=>i));
export const O20_SCENARIO='o20-normal-hit-transport-defense18-v1';
...
function payload(line){const i=line.indexOf('WPO20');return i<0?null:line.slice(i+5).trim();}
...
function classify(prev,cur){
  ...
  if(!(cur.orgHigh<prev.orgLow))return {multiplicity:0,loss};
  if(loss<.13)return {multiplicity:1,loss};
  if(loss<.225)return {multiplicity:2,loss};
  if(loss<.315)return {multiplicity:3,loss};
  return {multiplicity:4,loss};
}

export function parseO20Runs(text){
...
export function captureO20Run(run){
  const b=run?.begin||{};if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O20_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(b.defineOverrides!=='BASE_CHANCE_TO_AVOID_HIT:90,CHANCE_TO_AVOID_HIT_AT_NO_DEF:60,ORG_DICE:1,STR_DAMAGE:0,NIGHT_PENALTY:0')throw new Error('bad defines');
  if(b.attackModifier!=='army_infantry_attack_factor:-0.721'||b.defenderAttackModifier!=='army_infantry_attack_factor:-0.99'||b.defenseModifier!=='army_infantry_defence_factor:-0.9599742088'||b.prepared!=='yes'||b.modifiersPresent!=='yes')throw new Error('bad modifiers/preparation');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O20_HOURS))throw new Error('expected hours 0..161');
  ...
  for(let h=2;h<=161;h++){
    ...
    if(x.multiplicity===0)counts.zero++;else if(x.multiplicity===1)counts.one++;else if(x.multiplicity===2)counts.two++;else if(x.multiplicity===3)counts.three++;else counts.fourPlus++;
  }
  return {counts,intervals};
}

export function parseO20Batch(text){const candidates=parseO20Runs(text),accepted=[],rejected=[];candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO20Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});return {scenario:O20_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};}

async function cli(){const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o20-trial161.mjs <game.log>');process.exitCode=2;return;}try{process.stdout.write(JSON.stringify(parseO20Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}catch(error){console.error(`O20 parse failed: ${error.message}`);process.exitCode=1;}}
