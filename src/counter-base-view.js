...
export function renderCounterBase(host){
  ...
  host.innerHTML=`<section class="counter-hero"><div><p class="eyebrow">POST-GAME MATCHUP REVIEW</p><h2>Counter Analysis</h2><p>Recreate the matchup, then test focused one- and two-change design adjustments. Global settings are treated as fixed context; the search only modifies local template and equipment choices.</p></div><button class="primary" id="runCounterSearch">FIND COUNTERS</button></section>
