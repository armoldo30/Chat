...
export const O10_HOURS=Object.freeze(Array.from({length:62},(_,i)=>i));
export const O10_SCENARIO='o10-zero-defense-hit-gate-control-v1';
...
function payload(line){const i=line.indexOf('WPO10');return i<0?null:line.slice(i+5).trim();}
...
export function parseO10Runs(text){
...
export function captureO10Run(run){
  const b=run?.begin||{};
  if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O10_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O10_HOURS))throw new Error('expected hours 0..61');
...
export function parseO10Batch(text){
  const candidates=parseO10Runs(text),accepted=[],rejected=[];
  candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO10Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});
  return {scenario:O10_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};
}

async function cli(){
  const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o10-trial61.mjs <game.log>');process.exitCode=2;return;}
  try{process.stdout.write(JSON.stringify(parseO10Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}
  catch(error){console.error(`O10 parse failed: ${error.message}`);process.exitCode=1;}
}
