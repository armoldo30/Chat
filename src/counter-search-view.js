...
function productionNotes(item){
  ...
  if(plan?.available&&plan.incrementalIC>0){
    if(plan.currentPlanAdequate)notes.push(`CURRENT PLAN COVERED · The saved production plan has enough projected surplus beyond its existing targets to cover this force's added equipment within ${Math.round(plan.horizonDays)} days.`);
    else{
      const covered=Math.round((plan.coverageRatio||0)*100),missing=(plan.rows||[]).filter(row=>row.shortfall>1e-9).sort((a,b)=>b.shortfallIC-a.shortfallIC),missingLabels=missing.slice(0,2).map(row=>row.label).join(' / ');
      notes.push(`CURRENT PLAN GAP · About ${covered}% of the added equipment IC is covered beyond existing saved targets over ${Math.round(plan.horizonDays)} days; roughly ${fmt(plan.shortfallIC,0)} IC remains uncovered${missingLabels?` (${missingLabels})`:''}.`);
...
function resultRows(items,snap,config,search,side){
  ...
  return (items||[]).map((item,index)=>{const cost=economicBurden(baseIC,item.ic,config.baseMic,config.divisionCount),why=explainCounter(item,snap,side),costText=cost.delta>0?`≈ +${cost.extraFactories} MIC`:'no extra MIC',retool=item.practicality?.majorRetooling?' · major retooling':'',plan=item.productionPlan,production=plan?.available&&!plan.currentPlanAdequate?` · saved plan ${Math.round((plan.coverageRatio||0)*100)}% covered`:plan?.available&&plan.incrementalIC>0?' · saved plan covered':'';return `<article><div><b>#${index+1} ${esc(item.label)}</b><small>${item.changeCount||1} change${(item.changeCount||1)===1?'':'s'}${retool}${production} · ${esc(why.summary)}</small></div><span>${fmt(search.baseline.winRate,1)}% → ${fmt(item.winRate,1)}%${outcomeNote(item,search)} · ${signed(item.gain,1)} pp · ${cost.delta>=0?'+':''}${fmt(cost.delta,0)} modeled IC/div · ${costText}</span></article>`;}).join('');
}
