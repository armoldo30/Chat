...
function prettifyRegimentGroupLabels(){
  document.querySelectorAll('.regiment-title span,.hoi-picker .muted').forEach(node=>{
    const text=node.textContent||'',next=text.replaceAll('MOBILE_COMBAT_SUPPORT','MOBILE COMBAT SUPPORT').replaceAll('ARMOR_COMBAT_SUPPORT','ARMOR COMBAT SUPPORT').replaceAll('COMBAT_SUPPORT','COMBAT SUPPORT');
