...
const BASE_DIVISION_BATTALIONS=ordinaryDivisionBattalionIds(battalions);
...
function ensureDesignerState(side){
  const key=side+'Grid';
  if(!Array.isArray(state[key]))state[key]=countsToGrid(state[side],BASE_DIVISION_BATTALIONS,battalions);
  else state[key]=normalizeGrid(state[key],BASE_DIVISION_BATTALIONS,battalions);
  state[side]=gridToCounts(state[key],BASE_DIVISION_BATTALIONS);
...
function adjustedTankDesign(side,family,role='armor'){const base=buildTankDesign(tankDesignFor(side,family,role)),mio=tankMioFamily(family),target=tankVariantTargets(family,role);return mio?applyMioToVariant(base,mioEffectFor(side,mio,target?.equipmentKey||mio)):base;}
...
function syncDesignerSide(side){ensureDesignerState(side);state[side]=gridToCounts(state[side+'Grid'],BASE_DIVISION_BATTALIONS);const regKey=side+'RegimentalSupports';state[regKey]=state[regKey].map((key,c)=>key&&filledInRegiment(state[side+'Grid'],c)>=3&&regimentalBaselineCompatible(side,c,key)?key:null);}
...
function scenarioShareBaseline(){
  ...
  for(const side of ['attacker','defender']){
    base[side+'Grid']=countsToGrid(base[side],BASE_DIVISION_BATTALIONS,battalions);
...
function scenarioShareUrl(){
  if(!state.dataPack?.meta?.bundled)throw new Error(`Share links currently support the bundled vanilla ${MODEL_META.gameVersion} baseline only. Export JSON for custom data-pack scenarios.`);
...
async function copyScenarioShareLink(){
  try{
    ...
    if(clipboard?.writeText){
      ...
      globalThis.alert?.(`Matchup link copied. It includes the current inputs and designs on the bundled ${MODEL_META.gameVersion} baseline; cached battle results are not shared.`);
...
function division(side){ensureDesignerState(side);const data=techData(side),line=gridToCounts(state[side+'Grid'],BASE_DIVISION_BATTALIONS);return calcDivision(line,data.battalions,[...state[side+'Supports'],...validRegimentalSupports(side)],data.supports);}
...
function designerPicker(side){
  ...
  const battalionGroups=battalionPickerGroups(battalions,BASE_DIVISION_BATTALIONS);
...
function bindDivisionDesigner(side){
  ...
  document.querySelectorAll('[data-regimental-choice]').forEach(el=>el.onclick=()=>{
    const column=+el.dataset.regimentalColumn,value=el.dataset.regimentalChoice||null;
    assignRegimentalSupport(state,side,column,value,(candidate,c)=>regimentalBaselineCompatible(side,c,candidate));
  ...
  document.querySelectorAll('[data-choice]').forEach(el=>el.onclick=ev=>{
    ...
    if(designerPick?.kind==='battalion'){
      ...
      if(fill)state[side+'Grid']=fillRegiment(state[side+'Grid'],designerPick.c,value,BASE_DIVISION_BATTALIONS,battalions);
  ...
  $('exportDesigner').onclick=()=>downloadJSON(`${side}-division.json`,{name:state[side+'Name'],grid:state[side+'Grid'],battalions:gridToCounts(state[side+'Grid'],BASE_DIVISION_BATTALIONS),supports:state[side+'Supports'],regimentalSupports:state[side+'RegimentalSupports']});
  $('importDesigner').onchange=e=>{const f=e.target.files[0];if(!f)return;readJSON(f,x=>{state[side+'Grid']=Array.isArray(x.grid)?normalizeGrid(x.grid,BASE_DIVISION_BATTALIONS,battalions):countsToGrid(Array.isArray(x.battalions)?x.battalions:Array.isArray(x[side])?x[side]:[],BASE_DIVISION_BATTALIONS,battalions);state[side+'Supports']=normalizeSupportCompanySelection(Array.isArray(x.supports)?x.supports:Array.isArray(x[side+'Supports'])?x[side+'Supports']:[],supports,BASE_DIVISIONAL_SUPPORTS,5);state[side+'RegimentalSupports']=Array.isArray(x.regimentalSupports)?Array.from({length:DESIGNER_COLS},(_,i)=>{const raw=x.regimentalSupports[i],mapped=LEGACY_REGIMENTAL_MAP[raw]||raw;return BASE_REGIMENTAL_SUPPORTS.includes(mapped)?mapped:null;}):Array(DESIGNER_COLS).fill(null);if(x.name)state[side+'Name']=String(x.name).slice(0,80);syncDesignerSide(side);save();shell();});};
}
...
function airDesignPanel(key,label){
  ...
  const missionNote=d.source==='game-pack'?`<p class="muted"><b>Source-slot mode.</b> Structural compatibility, base module stats, IC, resources, weight and thrust use bundled game data. Aircraft mission-specific repeated stat blocks are not yet source-complete in the bundled import, so CAS/naval/strategic mission attack remains explicitly uncertified.</p>`:'';
...
function data(c){
  ...
  if($('clearPack'))$('clearPack').onclick=()=>{if(confirm(`Return to the bundled vanilla ${MODEL_META.gameVersion} baseline?`)){state.dataPack=null;save();location.reload();}};
}
...
function currentSavedMatchupToken(){
  if(!state.dataPack?.meta?.bundled)throw new Error(`Named saves currently support the bundled vanilla ${MODEL_META.gameVersion} baseline only. Export JSON to preserve a custom imported data pack.`);
...
function savedMatchupPanel(){
  ...
  return panel('Saved matchups',`${create}<div class="saved-matchup-count"><span>${savedMatchups.length}/${MAX_SAVED_MATCHUPS} stored in this browser</span><span>No account required</span></div>${savedMatchupLibraryError?`<p class="notice warn">${esc(savedMatchupLibraryError)}</p>`:''}<div class="saved-matchup-list">${rows||'<div class="saved-matchup-empty">No named matchups saved yet.</div>'}</div><p class="muted">Named saves use the same compact, version-locked inputs as share links. Cached battle results are excluded so a loaded matchup is recalculated under the bundled ${MODEL_META.gameVersion} baseline.</p>`,'saved-matchups-panel');
}
