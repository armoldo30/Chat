...
export const O11_HOURS=Object.freeze(Array.from({length:82},(_,i)=>i));
export const O11_SCENARIO='o11-defended-only-partition-v1';
...
function payload(line){const i=line.indexOf('WPO11');return i<0?null:line.slice(i+5).trim();}
function measure(fields,line){const out={};for(const name of ['orgLow','orgHigh','strengthLow','strengthHigh']){const n=finite(fields[name]);if(n===null||n<0||n>1)throw new Error(`line ${line}: invalid ${name}`);out[name]=n;}if(out.orgLow>out.orgHigh||out.strengthLow>out.strengthHigh)throw new Error(`line ${line}: reversed bounds`);return out;}
...
export function parseO11Runs(text){
...
export function captureO11Run(run){
  const b=run?.begin||{};if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O11_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O11_HOURS))throw new Error('expected hours 0..81');
  ...
  for(let i=2;i<run.samples.length;i++){
    const prev=run.samples[i-1].defenders[0],cur=run.samples[i].defenders[0],loss=lossPp(prev,cur),positive=strictLoss(prev,cur);
...
export function parseO11Batch(text){const candidates=parseO11Runs(text),accepted=[],rejected=[];candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO11Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});return {scenario:O11_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};}

async function cli(){const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o11-trial81.mjs <game.log>');process.exitCode=2;return;}try{process.stdout.write(JSON.stringify(parseO11Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}catch(error){console.error(`O11 parse failed: ${error.message}`);process.exitCode=1;}}
