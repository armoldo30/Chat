...
const bucketIds=new Set(Object.values(catalog).filter(x=>x&&typeof x==='object'&&!Array.isArray(x)).flatMap(map=>Object.keys(map||{})));
...
for(const [id,e] of tankish){
  const slots=Object.fromEntries(Object.entries(e.moduleSlots||{}).map(([slot,s])=>[slot,{required:s?.required,allowed:s?.allowed_module_categories||[]}]))
...
for(const [,e] of tankish)for(const s of Object.values(e.moduleSlots||{}))for(const c of s?.allowed_module_categories||[])allAllowed.add(c);
const moduleCats={};
