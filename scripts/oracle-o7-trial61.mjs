...
export const O7_HOURS=Object.freeze(Array.from({length:62},(_,i)=>i));
export const O7_SCENARIO='o7-wide-random-rounding-v1';
...
function payload(line){const i=line.indexOf('WPO7');return i<0?null:line.slice(i+4).trim();}
...
export function parseO7Runs(text){
...
export function captureO7Run(run){
  const b=run?.begin||{};
  if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O7_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(b.attackModifier!=='army_infantry_attack_factor:-0.721'||b.prepared!=='yes'||b.modifierPresent!=='yes')throw new Error('bad modifier/preparation');
  if(run.end.reason!=='trial61-complete'||Number(run.end.hour)!==61||run.end.modifierRemoved!=='yes'||run.end.cleanupFailure==='yes')throw new Error('bad END/cleanup');
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O7_HOURS))throw new Error('expected hours 0..61');
  ...
  for(const x of intervals){
    if(x.multiplicity===0)counts.zero++;
    else if(x.multiplicity===1)counts.one++;
    else if(x.multiplicity===2)counts.two++;
    else if(x.multiplicity===3)counts.three++;
    else counts.fourPlus++;
  }
  return {intervals,counts};
}

export function parseO7Batch(text){
  const candidates=parseO7Runs(text),accepted=[],rejected=[];
  candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO7Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});
  return {scenario:O7_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};
}

async function cli(){
  const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o7-trial61.mjs <game.log>');process.exitCode=2;return;}
  try{process.stdout.write(JSON.stringify(parseO7Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}
  catch(error){console.error(`O7 parse failed: ${error.message}`);process.exitCode=1;}
}
