...
export const O14_HOURS=Object.freeze(Array.from({length:82},(_,i)=>i));
export const O14_SCENARIO='o14-normal-defended-hit-v1';
...
function payload(line){const i=line.indexOf('WPO14');return i<0?null:line.slice(i+5).trim();}
...
function classify(prev,cur){const loss=(mid(prev)-mid(cur))*100;if(!(cur.orgHigh<prev.orgLow))return {multiplicity:0,loss};if(loss<.13)return {multiplicity:1,loss};return {multiplicity:2,loss};}

export function parseO14Runs(text){
...
export function captureO14Run(run){
  const b=run?.begin||{};if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O14_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(b.defineOverrides!=='BASE_CHANCE_TO_AVOID_HIT:90,CHANCE_TO_AVOID_HIT_AT_NO_DEF:100,ORG_DICE:1,STR_DAMAGE:0,NIGHT_PENALTY:0')throw new Error('bad defines');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O14_HOURS))throw new Error('expected hours 0..81');
  ...
  const counts={zero:0,one:0,twoPlus:0},intervals=[];
  for(let h=2;h<=81;h++){const x=classify(run.samples[h-1].defenders[0],run.samples[h].defenders[0]);intervals.push({fromHour:h-1,toHour:h,...x});if(x.multiplicity===0)counts.zero++;else if(x.multiplicity===1)counts.one++;else counts.twoPlus++;}
  return {counts,intervals};
}
export function parseO14Batch(text){const candidates=parseO14Runs(text),accepted=[],rejected=[];candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO14Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});return {scenario:O14_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};}

async function cli(){const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o14-trial81.mjs <game.log>');process.exitCode=2;return;}try{process.stdout.write(JSON.stringify(parseO14Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}catch(error){console.error(`O14 parse failed: ${error.message}`);process.exitCode=1;}}
