...
assert.equal(applied.battalions.infantry.speed,4.2);
...
assert.equal(applied.global.planningSpeed,.2);
assert.equal(applied.global.armySpeed,.1);
...
assert.ok(Math.abs(milestone.global.planningSpeed-.3)<1e-12,'grand doctrine milestone is selected by the real track order');

const air=structuredClone(DEFAULT_AIR_DOCTRINE);air.tracks.fighter_aircraft.mastery=1;
const design={size:'small',roles:['fighter'],agility:60,airAttack:20,airDefense:5,maxSpeed:400,range:700,reliability:.8,groundAttack:0,navalAttack:0};
const fx=airDoctrineEffects(air,design,pack);
assert.equal(fx.source,'game-pack');
assert.equal(fx.mission.air_superiority,.1);
assert.equal(fx.detection,.2);
