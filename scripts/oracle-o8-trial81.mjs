...
export const O8_HOURS=Object.freeze(Array.from({length:82},(_,i)=>i));
export const O8_SCENARIO='o8-defense-wide-rounding-v1';
...
function payload(line){const i=line.indexOf('WPO8');return i<0?null:line.slice(i+4).trim();}
...
export function parseO8Runs(text){
...
export function captureO8Run(run){
  const b=run?.begin||{};
  if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O8_SCENARIO)throw new Error('bad schema/scenario');
...
export function parseO8Batch(text){
  const candidates=parseO8Runs(text),accepted=[],rejected=[];
  candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO8Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});
  return {scenario:O8_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};
}

async function cli(){
  const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o8-trial81.mjs <game.log>');process.exitCode=2;return;}
  try{process.stdout.write(JSON.stringify(parseO8Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}
  catch(error){console.error(`O8 parse failed: ${error.message}`);process.exitCode=1;}
}
