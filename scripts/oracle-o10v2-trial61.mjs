...
export const O10V2_HOURS=Object.freeze(Array.from({length:62},(_,i)=>i));
export const O10V2_SCENARIO='o10v2-minimum-defense-response-v1';
...
function payload(line){const i=line.indexOf('WPO10V2');return i<0?null:line.slice(i+7).trim();}
function measure(fields,line){
  const out={}; for(const name of ['orgLow','orgHigh','strengthLow','strengthHigh']){const n=finite(fields[name]);if(n===null||n<0||n>1)throw new Error(`line ${line}: invalid ${name}`);out[name]=n;}
  if(out.orgLow>out.orgHigh||out.strengthLow>out.strengthHigh)throw new Error(`line ${line}: reversed bounds`); return out;
}
...
function classify(loss,positive){if(!positive)return 0;if(loss<0.13)return 1;if(loss<0.225)return 2;if(loss<0.315)return 3;return 4;}

export function parseO10V2Runs(text){
  const runs=[];let run=null,current=null;
  for(const [i,lineText] of String(text??'').split(/\r?\n/).entries()){
    const p=payload(lineText);if(p===null)continue; const j=p.indexOf(' '),type=j<0?p:p.slice(0,j),rest=j<0?'':p.slice(j+1),fields=kv(rest),line=i+1;
...
export function captureO10V2Run(run){
  const b=run?.begin||{}; if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O10V2_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(b.attackModifier!=='army_infantry_attack_factor:-0.721'||b.defenseModifier!=='army_infantry_defence_factor:-1.0328836429'||b.prepared!=='yes'||b.modifiersPresent!=='yes')throw new Error('bad modifiers/preparation');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O10V2_HOURS))throw new Error('expected hours 0..61');
  ...
  const intervals=[]; for(let i=2;i<run.samples.length;i++){const prev=run.samples[i-1].defenders[0],cur=run.samples[i].defenders[0],loss=lossPp(prev,cur),positive=strictLoss(prev,cur);intervals.push({fromHour:i-1,toHour:i,orgLossPp:loss,positive,multiplicity:classify(loss,positive)});}
  const counts={zero:0,one:0,two:0,three:0,fourPlus:0}; for(const x of intervals){if(x.multiplicity===0)counts.zero++;else if(x.multiplicity===1)counts.one++;else if(x.multiplicity===2)counts.two++;else if(x.multiplicity===3)counts.three++;else counts.fourPlus++;}
  return {intervals,counts};
}
export function parseO10V2Batch(text){const candidates=parseO10V2Runs(text),accepted=[],rejected=[];candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO10V2Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});return {scenario:O10V2_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};}
async function cli(){const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o10v2-trial61.mjs <game.log>');process.exitCode=2;return;}try{process.stdout.write(JSON.stringify(parseO10V2Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}catch(error){console.error(`O10v2 parse failed: ${error.message}`);process.exitCode=1;}}
