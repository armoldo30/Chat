...
import {parseO2Batch,O2_HOURS} from '../scripts/oracle-o2-trial12.mjs';
...
function run(scale=1){
  const lines=[
    'WPO2 BEGIN schema=1 scenario=o2-defended-amplified-v1 gameVersion=1.19.3.0.c01a checksum=5632 checksumScope=base-game-reference method=bisection14 runMode=trial12 tacticMode=neutral-basic-only amplifier=army_infantry_attack_factor:+2.0'
  ];
  ...
  lines.push('WPO2 END hour=12 reason=trial12-complete amplifierRemoved=yes');
  return lines.join('\n')+'\n';
}
...
assert.equal(result.runs[0].capture.samples.length,13);
...
console.log('Oracle O2 trial12 parser regression passed.');
