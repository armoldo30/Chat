...
function ensureDesignerState(side){
  ...
  state[side+'Supports']=(Array.isArray(state[side+'Supports'])?state[side+'Supports']:[]).filter(x=>BASE_DIVISIONAL_SUPPORTS.includes(x)).slice(0,5);
...
function applyFamilyMioToData(data,side){
  const maps={
    infantry_equipment:{b:['infantry','motorized','mechanized','cavalry'],s:['fire_support','mot_fire_support']},
    artillery:{b:['artillery'],s:['support_artillery','field_guns']},
    anti_tank:{b:['anti_tank'],s:['support_at','anti_tank_battery']},
    anti_air:{b:['anti_air'],s:['support_aa','anti_air_battery']}
  };
...
function scenarioShareBaseline(){
  ...
  for(const side of ['attacker','defender']){
    base[side+'Grid']=countsToGrid(base[side],Object.keys(battalions),battalions);
...
function bindDivisionDesigner(side){
  ...
  document.querySelectorAll('[data-choice]').forEach(el=>el.onclick=ev=>{
    ...
    if(designerPick?.kind==='battalion'){
      ...
      if(fill)state[side+'Grid']=fillRegiment(state[side+'Grid'],designerPick.c,value,Object.keys(battalions),battalions);
