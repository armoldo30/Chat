...
assert.match(js,/function syncSelection\(/,'visual choice strips must have a single source-of-truth selection synchronizer');
assert.match(js,/classList\.toggle\('selected',selected\)/,'visual choices must remove stale selected classes when the source value changes');
assert.match(js,/setAttribute\('aria-pressed',selected\?'true':'false'\)/,'visual choices must expose synchronized pressed state');
assert.match(js,/select\.addEventListener\('change',\(\)=>syncSelection/,'visual choices must also follow source-select changes');
...
assert.match(battlefield,/function syncPips\(/,'battlefield pip controls must have a source-of-truth synchronizer');
assert.match(battlefield,/classList\.toggle\('selected',selected\)/,'battlefield pips must clear stale selected classes');
assert.match(battlefield,/source\.addEventListener\('change',\(\)=>syncPips/,'battlefield pips must follow source-input changes');
