...
function card(title,item,snap,config){
  if(!item)return `<article class="counter-result empty"><span>${title}</span><h3>No qualifying two-step result</h3><p>The current focused search did not find a one- or two-change force design that improved modeled win rate by at least 2 percentage points.</p></article>`;
