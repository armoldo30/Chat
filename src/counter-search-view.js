...
function card(title,item,snap,config,search,side,note=''){
  if(!item)return '';
  const baseIC=snap[side+'IC'],cost=economicBurden(baseIC,item.ic),explanation=explainCounter(item,snap,side),notes=[note].filter(Boolean).map(text=>`<p class="counter-force-cost">${esc(text)}</p>`).join(''),quality=item.battleQuality,exchange=quality?`<b>Strength loss: own ${fmt(quality.ownCasualtyRate,1)}% · enemy ${fmt(quality.enemyCasualtyRate,1)}% · exchange ${signed(quality.casualtyExchange,1)} pp</b>`:'';
  return `<article class="counter-result"><span>${esc(title)} · ${item.changeCount||1} CHANGE${(item.changeCount||1)===1?'':'S'}</span><h3>${esc(item.label)}</h3><div class="counter-result-metrics"><b>${fmt(search.baseline.winRate,1)}% → ${fmt(item.winRate,1)}% modeled ${sideLabel(side)} win rate${outcomeNote(item,search)}</b><b>${signed(item.gain,1)} pp improvement</b><b>${fmt(cost.baselineIC,0)} → ${fmt(cost.candidateIC,0)} modeled IC/div</b><b>${signed(cost.delta,0)} IC · ${signed(cost.pct,1)}%</b>${exchange}</div>${notes}<div class="counter-explain"><strong>Why it works</strong>${list(explanation.reasons)}<strong>Tradeoffs</strong>${list(explanation.tradeoffs)}</div></article>`;
}
...
function resultRows(items,snap,config,search,side){
  ...
  return (items||[]).map((item,index)=>{const cost=economicBurden(baseIC,item.ic),why=explainCounter(item,snap,side),exchange=item.battleQuality?` · strength exchange ${signed(item.battleQuality.casualtyExchange,1)} pp`:'';return `<article><div><b>#${index+1} ${esc(item.label)}</b><small>${item.changeCount||1} change${(item.changeCount||1)===1?'':'s'} · ${esc(why.summary)}</small></div><span>${fmt(search.baseline.winRate,1)}% → ${fmt(item.winRate,1)}%${outcomeNote(item,search)} · ${signed(item.gain,1)} pp${exchange} · ${fmt(cost.baselineIC,0)} → ${fmt(cost.candidateIC,0)} IC/div (${signed(cost.pct,1)}%)</span></article>`;}).join('');
}
