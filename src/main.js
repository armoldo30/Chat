...
const defaults={
  ...
  production:{days:180,factories:30,efficiency:10,efficiencyGain:100,maxEfficiency:50,outputBonus:0,baseFactoryOutput:3.5,resources:{steel:60,aluminum:20,rubber:20,tungsten:15,chromium:5}},
  productionGoals:[
...
function requirementBadge(record){const req=prerequisiteText(record);return req?`<em class="req-info" title="${esc('Requirements (informational only): '+req)}">ⓘ req</em>`:'';}
...
function supportSlot(side,i){
  ...
  return `<button class="hoi-support-slot filled tone-${battalionTone(key)} ${unmet?'tech-prereq':''}" data-sslot="${i}" title="${esc(supports[key]?.name||key)}${unmet?' · prerequisite not selected (informational)':''}"><span>${SUPPORT_CODES[key]||'SUP'}</span><small>${esc(supports[key]?.name||key)}${unmet?' · REQ':''}</small></button>`;
}
function regimentalSupportSlot(side,c,filled){
  ...
  const unmet=!techAvailable('support',key,state[side+'Tech']);
  return `<button class="regimental-support available filled tone-${battalionTone(key)} ${unmet?'tech-prereq':''}" data-rslot="${c}" title="${esc(supports[key]?.name||key)}${unmet?' · prerequisite not selected (informational)':''}"><span>${SUPPORT_CODES[key]||'SUP'}</span><small>${esc(supports[key]?.name||key)}${unmet?' · REQ':''}</small></button>`;
}
function regimentColumn(side,c){
  const grid=state[side+'Grid'],filled=filledInRegiment(grid,c);
  const group=regimentGroup(side,c);return `<div class="hoi-regiment"><div class="regiment-title"><span>REGIMENT ${c+1}${group?` · ${group.toUpperCase()}`:''}</span><b>${filled}/5</b></div><div class="regiment-slots">${grid[c].map((type,r)=>{if(!type)return `<button class="hoi-battalion-slot empty" data-bslot="1" data-c="${c}" data-r="${r}" title="Add battalion"><span>+</span><small>Add</small></button>`;const locked=!techAvailable('battalion',type,state[side+'Tech']);return `<button class="hoi-battalion-slot filled tone-${battalionTone(type)} ${locked?'tech-prereq':''}" data-bslot="1" data-c="${c}" data-r="${r}" title="${esc(battalions[type].name)}${locked?' · prerequisite (informational)':''}"><span class="unit-symbol">${BATTALION_CODES[type]||'BAT'}</span><small>${esc(battalions[type].name)}${locked?' · REQ':''}</small></button>`;}).join('')}</div>${regimentalSupportSlot(side,c,filled)}</div>`;
}
