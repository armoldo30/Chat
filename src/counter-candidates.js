...
export function buildCounterCandidates(snapshot,{side='attacker',grid=null,supportKeys=null,priorChanges=[],priorKinds=[],limit=80}={}){
  const {state}=snapshot,baseGrid=grid||state[side+'Grid'],baseSupports=supportKeys||state[side+'Supports'],candidates=[],seen=new Set([counterTemplateKey(baseGrid,baseSupports)]);
  const add=(nextGrid,nextSupports,description,kind)=>{
    ...
    const changes=[...priorChanges,description],changeKinds=[...priorKinds,kind];candidates.push({side,grid:nextGrid,supportKeys:nextSupports,changes,changeKinds,label:changeLabel(changes),kind,changeCount:changes.length,key});
  };
