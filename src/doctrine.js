...
function importedUnitScopeMatches(scope,id,unit){
  if(!scope||!unit)return false;
  const gameId=unit.gameId||id,categories=unit.categories||[],types=unit.types||[];
  return scope===id||scope===gameId||categories.includes(scope)||types.includes(scope);
}
...
function applyImportedLandNode(node,battalions,supports,global,mods){
  if(!node||typeof node!=='object'||Array.isArray(node))return;
  for(const [key,value] of Object.entries(node)){
    if(DOCTRINE_META_KEYS.has(key))continue;
    if(Number.isFinite(Number(value))&&IMPORTED_GLOBAL_MAP[key]){const dst=IMPORTED_GLOBAL_MAP[key];global[dst]=(global[dst]||0)+Number(value);continue;}
    if(!value||typeof value!=='object'||Array.isArray(value))continue;
    for(const [id,unit] of Object.entries(battalions))if(importedUnitScopeMatches(key,id,unit))collectImportedUnitBlock(unit,value,mods);
...
function importedLandDoctrineApply(battalions,supports,raw,pack){
  ...
  if(grand?.raw){applyImportedLandNode(grand.raw,b,s,global,mods);used.push(grandId);}
  for(const [track,t] of Object.entries(state.tracks)){
    const id=importedSubDoctrineId(t.choice,'land',pack),doc=id?pack.doctrines[id]:null;if(!doc?.raw)continue;used.push(id);applyImportedLandNode(doc.raw,b,s,global,mods);
...
export function applyLandDoctrineToData(battalions,supports,raw,pack=null){
  if(pack?.doctrines&&Object.keys(pack.doctrines).length)return importedLandDoctrineApply(battalions,supports,raw,pack);
  const {state,effects}=landDoctrineEffects(raw,pack),b=clone(battalions),s=clone(supports);
