...
function noCounterPanel(search,snap,config,side){
  ...
  let diagnosis='The bounded search screened the current structural counter catalog but did not find a one- or two-change answer large enough to count as a meaningful improvement.';
  if(base<35)diagnosis='This matchup is heavily unfavorable. The browser-safe one/two-change search did not find a meaningful local counter; force ratio, battlefield conditions, or a larger redesign may be the limiting factor.';
  ...
  return `<article class="counter-result empty"><span>LOCAL COUNTER NOT FOUND</span><h3>No meaningful one- or two-change counter emerged</h3><p>Baseline modeled ${sideLabel(side)} win rate: ${fmt(base,1)}%. ${esc(attempt)}</p><p>${esc(diagnosis)}</p></article>${effort}`;
}
