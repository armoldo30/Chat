...
function designerPicker(side){
  ...
  if(designerPick.kind==='support'){
    const current=state[side+'Supports'][designerPick.i],selectedSupports=state[side+'Supports'].filter(Boolean),groups=supportCompanyPickerGroups(supports,BASE_DIVISIONAL_SUPPORTS);
    return `<div class="hoi-picker"><div class="picker-head"><div><span class="eyebrow">MAKE A SELECTION</span><h3>Support Companies</h3><p class="muted">Grouped by battlefield role and alphabetized within each group.</p></div><button class="btn" data-cancel-pick>Back</button></div><section class="picker-group"><div class="picker-grid"><button class="picker-choice remove-choice" data-choice="">×<small>Empty slot</small></button></div></section>${Object.entries(groups).map(([name,ids])=>`<section class="picker-group"><h4>${esc(name)}</h4><div class="picker-grid">${ids.map(k=>{const v=supports[k],info=requirementInfo(v);return `<button class="picker-choice ${info?'prereq-info':''}" data-choice="${k}" ${supportChoiceBlocked(k,current,selectedSupports,supports)?'disabled':''} title="${esc(info)}"><b>${SUPPORT_CODES[k]||'SUP'}</b><small>${esc(v.name)}${info?' · ⓘ':''}</small>${pickerSupportMeta(side,k)}</button>`;}).join('')}</div></section>`).join('')}</div>`;
  }
