...
const [html,doctrine,airDoctrine,mio,inlineMio,runtime,css,airCss,mioCss]=await Promise.all([
...
for(const file of ['ui-localization-runtime.js','doctrine-board.js','air-doctrine-board.js','mio-board.js','inline-mio-guided.js'])assert.match(html,new RegExp(file.replace('.','\\.')),`index should load ${file}`);
for(const file of ['visual-boards.css','air-doctrine-board.css','mio-workflow.css'])assert.match(html,new RegExp(file.replace('.','\\.')),`index should load ${file}`);
assert.doesNotMatch(html,/src\/inline-mio-board\.js/,'legacy inline MIO visual board should not be loaded alongside the guided workflow');
...
assert.match(mio,/\[data-mio-trait/,'MIO board must reuse existing MIO trait controls');
...
assert.match(mio,/AVAILABLE NOW/,'full MIO board must surface the next valid trait choices');
assert.match(inlineMio,/AVAILABLE NOW/,'inline MIO board must surface the next valid trait choices');
...
assert.match(mioCss,/inline-mio-guided-launch/);
...
console.log('Visual doctrine/guided-MIO board regression checks passed.');
