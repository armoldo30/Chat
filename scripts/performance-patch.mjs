...
export function applyPerformancePatch(source) {
  ...
  out = replaceOnce(
    ...
    `function render(r){const v=$('view');({dashboard,battle,gauntlet,tank,air,production,front,intel,data,scenario}[r]||battle)(v);}\nfunction refreshDivisionLab(){const v=$('view');if(v&&route()==='battle')battle(v);else shell();}`,
    'render helper'
  );
  ...
  out = replaceOnce(out,
    ...
    "document.querySelectorAll('[data-bslot]').forEach(el=>el.onclick=ev=>{designerPick={kind:'battalion',side,c:+el.dataset.c,r:+el.dataset.r,fillRegiment:!!ev.shiftKey};refreshDivisionLab();});",
  ...
  out = replaceOnce(out,
    ...
    "document.querySelectorAll('[data-sslot]').forEach(el=>el.onclick=()=>{designerPick={kind:'support',side,i:+el.dataset.sslot};refreshDivisionLab();});",
    'support picker open handler');

  out = replaceOnce(out,
    "document.querySelectorAll('[data-rslot]').forEach(el=>el.onclick=()=>{designerPick={kind:'regimental',side,c:+el.dataset.rslot};shell();});",
    "document.querySelectorAll('[data-rslot]').forEach(el=>el.onclick=()=>{designerPick={kind:'regimental',side,c:+el.dataset.rslot};refreshDivisionLab();});",
    'regimental picker open handler');

  out = replaceOnce(out,
    "document.querySelectorAll('[data-cancel-pick]').forEach(el=>el.onclick=()=>{designerPick=null;shell();});",
    "document.querySelectorAll('[data-cancel-pick]').forEach(el=>el.onclick=()=>{designerPick=null;refreshDivisionLab();});",
    'picker cancel handler');
  ...
  out = replaceOnce(out,
    ...
    "const pickerOverlay=document.querySelectorAll('[data-picker-overlay]')[0];if(pickerOverlay)pickerOverlay.onclick=e=>{if(e.target===pickerOverlay){designerPick=null;refreshDivisionLab();}};",
    'picker overlay handler');

  out = replaceOnce(out,
    "syncDesignerSide(side);designerPick=null;save();shell();",
    "syncDesignerSide(side);designerPick=null;save();refreshDivisionLab();",
...
async function main() {
  ...
  if (!target) throw new Error('Usage: node scripts/performance-patch.mjs <path-to-main.js>');
