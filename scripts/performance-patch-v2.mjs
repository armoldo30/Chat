...
export function applyPerformancePatch(source){
  ...
  out=replaceOnce(out,"function division(side){ensureDesignerState(side);const data=techData(side),line=gridToCounts(state[side+'Grid'],BASE_DIVISION_BATTALIONS);return calcDivision(line,data.battalions,[...state[side+'Supports'],...validRegimentalSupports(side)],data.supports);}",`const divisionSideCache=new Map();\nfunction division(side){\n  ensureDesignerState(side);\n  const sig=sideDerivedSignature(side)+'|'+JSON.stringify([state[side+'Grid'],state[side+'Supports'],state[side+'RegimentalSupports']]),cached=divisionSideCache.get(side);\n  if(cached&&cached.pack===state.dataPack&&cached.sig===sig)return cached.value;\n  const data=techData(side),line=gridToCounts(state[side+'Grid'],BASE_DIVISION_BATTALIONS),value=calcDivision(line,data.battalions,[...state[side+'Supports'],...validRegimentalSupports(side)],data.supports);\n  divisionSideCache.set(side,{pack:state.dataPack,sig,value});return value;\n}`,'division calculation');
