...
export const O15_HOURS=Object.freeze(Array.from({length:162},(_,i)=>i));
export const O15_SCENARIO='o15-normal-undefended-hit-v1';
...
function payload(line){const i=line.indexOf('WPO15');return i<0?null:line.slice(i+5).trim();}
...
function classify(prev,cur){const loss=(mid(prev)-mid(cur))*100;if(!(cur.orgHigh<prev.orgLow))return {multiplicity:0,loss};if(loss<.13)return {multiplicity:1,loss};if(loss<.225)return {multiplicity:2,loss};return {multiplicity:3,loss};}

export function parseO15Runs(text){
...
export function captureO15Run(run){
  const b=run?.begin||{};if(!run?.end)throw new Error('missing END');
  if(Number(b.schema)!==1||b.scenario!==O15_SCENARIO)throw new Error('bad schema/scenario');
  ...
  if(b.defineOverrides!=='BASE_CHANCE_TO_AVOID_HIT:100,CHANCE_TO_AVOID_HIT_AT_NO_DEF:60,ORG_DICE:1,STR_DAMAGE:0,NIGHT_PENALTY:0')throw new Error('bad defines');
  ...
  if(JSON.stringify(run.samples.map(s=>s.hour))!==JSON.stringify(O15_HOURS))throw new Error('expected hours 0..161');
  ...
  const counts={zero:0,one:0,two:0,threePlus:0},intervals=[];
  for(let h=2;h<=161;h++){const x=classify(run.samples[h-1].defenders[0],run.samples[h].defenders[0]);intervals.push({fromHour:h-1,toHour:h,...x});if(x.multiplicity===0)counts.zero++;else if(x.multiplicity===1)counts.one++;else if(x.multiplicity===2)counts.two++;else counts.threePlus++;}
  return {counts,intervals};
}
export function parseO15Batch(text){const candidates=parseO15Runs(text),accepted=[],rejected=[];candidates.forEach((r,i)=>{try{accepted.push({runNumber:i+1,capture:captureO15Run(r)});}catch(error){rejected.push({runNumber:i+1,reason:error.message});}});return {scenario:O15_SCENARIO,totalRuns:candidates.length,acceptedRuns:accepted.length,rejectedRuns:rejected.length,rejected,runs:accepted};}

async function cli(){const [,,input]=process.argv;if(!input){console.error('Usage: node scripts/oracle-o15-trial161.mjs <game.log>');process.exitCode=2;return;}try{process.stdout.write(JSON.stringify(parseO15Batch(await fs.readFile(input,'utf8')),null,2)+'\n');}catch(error){console.error(`O15 parse failed: ${error.message}`);process.exitCode=1;}}
